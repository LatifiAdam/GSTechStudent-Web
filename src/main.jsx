...
const NAV = {
  superadmin: [['dashboard','Tableau de bord',LayoutDashboard],['users','Utilisateurs',Users],['establishments','Établissements',Building2],['profile','Profil',CircleUserRound],['system','Maintenance',Settings]],
  df: [['dashboard','Tableau de bord',LayoutDashboard],['users','Utilisateurs',Users],['establishments','Établissements',Building2],['regions','Régions',Layers3],['profile','Profil',CircleUserRound]],
  srio: [['dashboard','Tableau de bord',LayoutDashboard],['establishments','Établissements',Building2],['managers','Gestionnaires',UserCheck],['profile','Profil',CircleUserRound]],
  scq: [['dashboard','Tableau de bord',LayoutDashboard],['establishments','Établissements',Building2],['directors','Directeurs',UserRound],['director-assign','Affectations',Layers3],['profile','Profil',CircleUserRound]],
  directeur: [['dashboard','Tableau de bord',LayoutDashboard],['formateurs','Formateurs',UserCheck],['courses','Modules',BookOpen],['assign','Affectations',Layers3],['schedule','Emploi du temps',CalendarDays],['documents','Documents',FileText],['announcements','Annonces',MessageSquare],['profile','Profil',CircleUserRound]],
  gestionnaire: [['dashboard','Tableau de bord',LayoutDashboard],['groups','Groupes',Layers3],['students','Stagiaires',GraduationCap],['documents','Documents',FileText],['justifications','Justificatifs',ClipboardCheck],['profile','Profil',CircleUserRound]],
  formateur: [['dashboard','Tableau de bord',LayoutDashboard],['schedule','Emploi du temps',CalendarDays],['attendance','Présences',ClipboardCheck],['grades','Notes',BarChart3],['announcements','Annonces',MessageSquare],['profile','Profil',CircleUserRound]],
...
function App(){
  ...
  const [openRole,setOpenRole]=useState(false); const [logoutConfirm,setLogoutConfirm]=useState(false);
  ...
  return <div className="app">
    <aside className={`sidebar ${mobile?'mobile-open':''}`}>
      <div className="brand"><img className="brand-logo" src="/logo-v5.png" alt="GSTechStudent" /><div><strong>GSTechStudent</strong><span>Plateforme Web</span></div><button className="icon-btn side-close" onClick={()=>setMobile(false)}><X size={18}/></button></div>
      ...
      <div className="sidebar-bottom"><button className="nav-item" onClick={()=>setDark(v=>!v)}>{dark?<Sun size={18}/>:<Moon size={18}/>}<span>{dark?'Mode clair':'Mode sombre'}</span></button><button className="nav-item danger" onClick={()=>setLogoutConfirm(true)}><LogOut size={18}/><span>Déconnexion</span></button></div>
    </aside>
    ...
    {toast&&<div className={`toast ${toast.type==='error'?'error':'success'}`}>{toast.type==='error'?<AlertCircle size={17}/>:<CheckCircle2 size={17}/>}<span>{toast.message}</span></div>}{logoutConfirm&&<Modal title="Confirmer la déconnexion" onClose={()=>setLogoutConfirm(false)}><p>Voulez-vous vraiment vous déconnecter ?</p><div className="modal-actions"><button className="secondary" onClick={()=>setLogoutConfirm(false)}>Annuler</button><button className="primary danger-btn" onClick={()=>{setLogoutConfirm(false);logout()}}>Déconnexion</button></div></Modal>}
  </div>
}
...
function Login({onLogin,onVerify,apiBase}){
  ...
  return <div className="login-page"><div className="login-orb orb-a"/><div className="login-orb orb-b"/><div className="login-card">
    <div className="login-brand"><img className="brand-logo big" src="/logo-v5.png" alt="GSTechStudent" /><div><strong>GSTechStudent</strong><span>Plateforme de gestion OFPPT</span></div></div>
...
function Dashboard({role,session,onNavigate}){
  ...
  const studentCountFromClasses=Array.isArray(classes.data)?classes.data.reduce((n,c)=>n+Number(c.effectif??c.nombreStagiaires??0),0):null;const counts={users:Array.isArray(users.data)?users.data.length:null,students:studentCountFromClasses,etabs:Array.isArray(etabs.data)?etabs.data.length:null,classes:Array.isArray(classes.data)?classes.data.length:null,courses:Array.isArray(courses.data)?courses.data.length:null};
  const cards=role==='stagiaire'
    ...
    : role==='superadmin'
      ...
      : role==='formateur' ? [['Stagiaires',counts.students??'—','Stagiaires accessibles',GraduationCap],['Classes',counts.classes??'—','Classes accessibles',Layers3],['Présences','—','Suivi pédagogique',ClipboardCheck],['Notes','—','Évaluations',BarChart3]] : [['Utilisateurs',canUsers?(counts.users??'—'):'—','Dans le périmètre',Users],['Établissements',canEtabs?(counts.etabs??'—'):'—','Dans le périmètre',Building2],['Groupes',canClasses?(counts.classes??'—'):'—','Classes visibles',Layers3],['Modules',canCourses?(counts.courses??'—'):'—','Modules visibles',BookOpen]];
...
function DirectorAssignments({role,search,notify}){const e=useApi('/etablissements');const d=useApi('/users?role=directeur');const rows=filterRows(e.data,search);const assign=async(efpId,directorId)=>{try{await api.post(`/etablissements/${efpId}/directeur`,{idUtilisateur:directorId});notify('success','Directeur affecté à l’établissement.');e.reload()}catch(err){notify('error',err.message)}};return <><PageTitle title="Affectation des Directeurs" subtitle="Affecter un Directeur de votre région à un EFP"/><DataPanel loading={e.loading||d.loading} error={e.error||d.error} reload={()=>{e.reload();d.reload()}}>{rows?.length?<div className="table-wrap"><table><thead><tr><th>Établissement</th><th>Région</th><th>Directeur</th></tr></thead><tbody>{rows.map(x=><tr key={x.idEtablissement}><td>{x.nomEtablissement}</td><td>{x.region||'—'}</td><td><select value={x.idDirecteur||''} onChange={ev=>{if(ev.target.value)assign(x.idEtablissement,ev.target.value)}}><option value="">Non affecté</option>{(d.data||[]).map(u=><option key={u.idUtilisateur} value={u.idUtilisateur}>{nameOf(u)}</option>)}</select></td></tr>)}</tbody></table></div>:<EmptyState text="Aucun établissement accessible."/>}</DataPanel></>}
...
function Documents({role,search,notify}){let path=role==='gestionnaire'?'/documents/mine':role==='stagiaire'?'/documents/approved':['df','directeur'].includes(role)?'/documents/all':null;const q=useApi(path);const rq=useApi(['gestionnaire','stagiaire','df'].includes(role)?'/document-requests':null);const docs=Array.isArray(q.data)?q.data:[];const reqs=Array.isArray(rq.data)?rq.data:[];const [upload,setUpload]=useState(false);return <><PageTitle title="Documents" subtitle="Documents et demandes selon votre périmètre" action={role==='gestionnaire'?'Créer un document':null} onAction={()=>setUpload(true)}/><div className="split-panels"><DataPanel loading={q.loading} error={q.error} reload={q.reload}><h3>Documents</h3>{docs.length?<Table headers={['Nom','Statut','ID']} rows={filterRows(docs,search).map(d=>[d.nomDocument||d.filename||d.nom||'—',d.statut||d.status||'—',d.idDocument||d.id||'—'])} actions={false}/>:<EmptyState text="Aucun document disponible."/>}</DataPanel><DataPanel loading={rq.loading} error={rq.error} reload={rq.reload}><h3>Demandes</h3>{reqs.length?<Table headers={['Type','Demandeur','Statut','ID']} rows={filterRows(reqs,search).map(d=>[d.typeDocument||d.nomDocument||d.type||'—',d.stagiaire?nameOf(d.stagiaire):d.requester||d.idStagiaire||'—',d.statut||d.status||'—',d.idDemande||d.id||'—'])} actions={false}/>:<EmptyState text="Aucune demande accessible."/>}</DataPanel></div>{upload&&<DocumentUploadModal onClose={()=>setUpload(false)} onDone={()=>{setUpload(false);q.reload();notify('success','Document envoyé au Directeur pour validation.')}}/>}</>}
function DocumentUploadModal({onClose,onDone}){const [name,setName]=useState('');const [file,setFile]=useState(null);const [busy,setBusy]=useState(false);const submit=async e=>{e.preventDefault();if(!file)return; if(file.type!=='application/pdf')return alert('Seuls les PDF sont autorisés.'); if(file.size>10*1024*1024)return alert('Le fichier ne doit pas dépasser 10 Mo.');setBusy(true);try{const fd=new FormData();fd.append('file',file,file.name);fd.append('nomDocument',name.trim()||file.name);await api.request('/documents/upload',{method:'POST',body:fd});onDone()}catch(err){alert(err.message)}finally{setBusy(false)}};return <Modal title="Créer un document" onClose={onClose}><form className="modal-form" onSubmit={submit}><Field label="Nom du document" value={name} onChange={setName} maxLength={255}/><label>Fichier PDF<input type="file" accept="application/pdf,.pdf" onChange={e=>setFile(e.target.files?.[0]||null)} required/></label>{file&&<small>{file.name} · {(file.size/1024/1024).toFixed(2)} Mo</small>}<div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Annuler</button><button className="primary" disabled={busy||!file}>{busy?'Envoi...':'Envoyer au Directeur'}</button></div></form></Modal>}
...
function Announcements({role,search,notify}){const q=useApi(['formateur','stagiaire','df','directeur'].includes(role)?'/announcements':null);const rows=filterRows(q.data,search);const [create,setCreate]=useState(false);return <><PageTitle title="Annonces" subtitle="Communication pédagogique et administrative" action={['df','directeur'].includes(role)?'Publier':null} onAction={()=>setCreate(true)}/><DataPanel loading={q.loading} error={q.error} reload={q.reload}>{rows?.length?<div className="announcement-list">{rows.map((a,i)=><div className="announcement" key={a.idAnnonce||i}><div className="announce-icon"><Bell size={18}/></div><div><strong>{a.titre||a.title||'Annonce'}</strong><p>{a.contenu||a.message||a.description||'—'}</p><small>{a.datePublication||a.createdAt||'—'}</small></div></div>)}</div>:<EmptyState text="Aucune annonce disponible."/>}</DataPanel>{create&&<AnnouncementModal onClose={()=>setCreate(false)} onDone={()=>{setCreate(false);q.reload();notify('success','Annonce publiée.')}} role={role}/>}</>}
function AnnouncementModal({onClose,onDone,role}){const [title,setTitle]=useState('');const [content,setContent]=useState('');const [busy,setBusy]=useState(false);const submit=async e=>{e.preventDefault();if(!title.trim()||!content.trim())return;setBusy(true);try{await api.post('/announcements',{titre:title.trim(),contenu:content.trim(),typeAnnonce:'generale'});onDone()}catch(err){alert(err.message)}finally{setBusy(false)}};return <Modal title="Publier une annonce" onClose={onClose}><form className="modal-form" onSubmit={submit}><Field label="Titre" value={title} onChange={setTitle} required maxLength={255}/><label>Contenu<textarea value={content} onChange={e=>setContent(e.target.value)} rows={6} required maxLength={5000}/></label><div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Annuler</button><button className="primary" disabled={busy}>{busy?'Publication...':'Publier'}</button></div></form></Modal>}

function Profile({session}){const q=useApi(`/users/${session.sub}`);const u=q.data;const [image,setImage]=useState(null);const [busy,setBusy]=useState(false);useEffect(()=>{let revoke; if(u?.profileImageKey){api.get(`/users/${session.sub}/profile-image`).then(blob=>{revoke=URL.createObjectURL(blob);setImage(revoke)}).catch(()=>{})} return()=>{if(revoke)URL.revokeObjectURL(revoke)}},[u?.profileImageKey,session.sub]);const upload=async e=>{const file=e.target.files?.[0];if(!file)return;if(!['image/jpeg','image/png','image/webp'].includes(file.type))return alert('Format accepté : JPG, PNG ou WEBP.');if(file.size>2*1024*1024)return alert('L’image ne doit pas dépasser 2 Mo.');setBusy(true);try{const fd=new FormData();fd.append('file',file,file.name);await api.request('/users/me/profile-image',{method:'POST',body:fd});q.reload()}catch(err){alert(err.message)}finally{setBusy(false)}};return <><PageTitle title="Profil" subtitle="Informations du compte connecté"/><DataPanel loading={q.loading} error={q.error} reload={q.reload}>{u?<><div className="profile-card"><div className="profile-avatar">{image?<img src={image} alt="Photo de profil"/>:<>{(u.prenom?.[0]||'G')}{(u.nom?.[0]||'S')}</>}</div><div><h2>{nameOf(u)}</h2><p>{u.email}</p><span className="badge success">Compte actif</span><div style={{marginTop:10}}><label className="secondary" style={{cursor:'pointer'}}>{busy?'Upload...':'Modifier la photo'}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={upload} hidden/></label></div></div></div><div className="grid-2"><div className="panel inner"><h2>Informations</h2><div className="info-grid"><label>Rôle<strong>{roleLabel(u.role)}</strong></label><label>Région<strong>{u.region||'National'}</strong></label><label>Téléphone<strong>{u.telephone||'—'}</strong></label><label>CIN<strong>{u.cin||'—'}</strong></label></div></div><div className="panel inner"><h2>Sécurité</h2><div className="security-row"><ShieldCheck size={18}/><div><strong>Session JWT</strong><small>Access token 15 min · refresh token 7 jours</small></div><CheckCircle2 size={18}/></div></div></div></>:<EmptyState text="Profil introuvable."/>}</DataPanel></>}
